...
const HRDashboard = () => {
  ...
  // Removed unused 'loading' state to fix Vercel Build Error
  const [showTeamView, setShowTeamView] = useState(false);
  const [chartTab, setChartTab] = useState('energy'); 

  // Wrapped in useCallback and used Env Variable for API URL
  ...
  // useEffect now has proper dependency
  useEffect(() => { 
    refreshData(); 
  }, [refreshData]);
          ...
          <div className="h-[320px] bg-slate-800/40 backdrop-blur-xl rounded-3xl border border-white/10 p-5 flex flex-col shadow-xl relative overflow-hidden">
            ...
            <div className="flex-1 w-full min-h-0 relative z-10">
              {chartTab === 'energy' ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={driverStats} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value" nameKey="name" stroke="none">
                      {driverStats.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                    <Tooltip content={<CustomPieTooltip />} />
                    <Legend verticalAlign="middle" align="right" layout="vertical" iconSize={8} wrapperStyle={{ fontSize: '12px', fontWeight: 'bold', color: '#cbd5e1' }} />
                  </PieChart>
                </ResponsiveContainer>
              )}
            </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="bg-white/5 rounded-xl p-3 border border-white/5 flex items-center gap-3">
                       ...
                       <div>
                         ...
                         <div className="text-sm font-bold text-white">{selectedEmp.primary_driver || "Unknown"}</div>
                       </div>
                    </div>
